...
-- Change the brightness value of `to_change` based on the brightness of
-- `basis`.
...
local function increase_contrast(to_change, basis)
    return is_bright(basis) and to_change:darken(0.75)
        or to_change:lighten(0.75)
end

-- Exaggerates the brightness value of a color. If a color is already bright,
-- the returned color will be a brighter version of it, and vice versa
---@param color Color color to be brightened/darkened
---@return Color color2
...
-- Color picking is based on Kanagawa Dragon's structure, not just randomly or
-- different for each category. The rules goes as follows:
...
---@return TabBar colorscheme tab bar colorscheme
function M.build_tab_bar_colorscheme_from_image(image)
    ---@type table<number, Color>
    local color_from_image =
        wezterm.color.extract_colors_from_image(image.path or image, {
