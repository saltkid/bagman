...
---@param object_fit "contain" | "cover" | "fill" | "none" | "scale-down"
...
function M.resize_image(image_width, image_height, window_width, window_height, object_fit)
	...
	elseif "scale-down" == object_fit then
		local new_width, new_height = M.scale_down_dimensions(image_width, image_height, window_width, window_height)
		return new_width, new_height
	else
...
---Computes css's `object-fit: scale-down` width and height of an image using current window width
---and height as basis.
---This does either `object-fit: none` or `object-fit: contain`, whichever one is smaller.
---@param image_width number
---@param image_height number
---@param window_width number
---@param window_height number
---@return number width
---@return number height
function M.scale_down_dimensions(image_width, image_height, window_width, window_height)
	if image_width > window_width or image_height > window_height then
		local width_val, height_val = M.contain_dimensions(image_width, image_height, window_width, window_height)
